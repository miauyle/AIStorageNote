...
适合经过筛选的冷 prefix 共享，通常不作为低延迟 Decode 的逐页默认来源；比较恢复与重算。

**第二层：怎么判断值得？**

估算 KV bytes、有效带宽和固定开销，加入 layout/sync 与 queue；结合复用概率、预计 GPU 重算时间及 SLO。

**第三层：RDMA 超时后会怎样？**

失败 attempt 不发布。旧 target 隔离，直到硬件访问安全结束；重试用独立 generation/allocation。不要把“KV 可重算”当忽略 buffer 安全的理由。

你过去的故障恢复经验在第三层开始真正产生优势：不仅能解释新概念，还能指出大规模异步数据移动的失效模式。
...
> 我的主要经验在分布式对象存储的数据移动、复制、恢复和线上问题处理。AI Storage 新增的是 GPU 消费端的容量、布局、带宽与同步约束。我能把这些约束落到对象身份、后台搬运、流控、失败隔离和可观测性上。对于 CUDA/RDMA，我已经建立数据路径和资源生命周期模型，并用明确区分模拟与实机的实验验证；尚未实机完成的部分我会说明边界。
...
第一版可以用 Python 控制逻辑加 bytes/memoryview 以快速完成实验；接口保持语言无关。后续真实 CUDA/RDMA 适配用 C++/CUDA 与 RAII 封装。语言选择服务于验证，不用第一版就承担完整 C++ S3 SDK 构建和 CUDA 兼容成本。
...
闭卷完成以下五件事即可开始有针对性投递，不以完成真实 RDMA Demo 作为门槛：
